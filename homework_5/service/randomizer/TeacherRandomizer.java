package homework_5.service.randomizer;

import lombok.Data;
import lombok.EqualsAndHashCode;
import homework_5.service.datareader.DataReader;
    ...
    public List<String> getData() {
        ...
        if (gender == 0) {
            fullName = getLoadedWomenFirstNames().get(random.nextInt(getLoadedWomenFirstNames().size()))
                    + " "
                    + getLoadedWomenSurname().get(random.nextInt(getLoadedWomenSurname().size()));
        ...
        if (gender == 1) {
            fullName = getLoadedMenFirstNames().get(random.nextInt(getLoadedMenFirstNames().size()))
                    + " "
                    + getLoadedMenSurname().get(random.nextInt(getLoadedMenSurname().size()));
            age = random.nextInt(27, 67);
            phoneNumber = PhoneNumberRandomizer.getPhoneNumber();
        }
